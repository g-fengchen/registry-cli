 ...
 */
export async function checkBuildAvailable(
  serviceName: string,
  functionName: string,
  baseDir = process.cwd()
) {
  ...
  if (status === "unavailable") {
    throw new Error(
      `${serviceName}/${functionName} build status is unavailable.Please re-execute 's build'`
    );
  }
}
...
export function transfromTriggerConfig(triggerConfig, region, accountId) {
  const { name, type, config, qualifier, role, sourceArn } = triggerConfig;
  ...
  if (type === "oss") {
    ...
    }:${accountId}:/topics/${config.topicName}`;
...
export function getTargetTriggers(
  sourceTriggers: any[],
  onlyDelpoyTriggerName: string | string[]
) {
  let needDeployTriggers = [];
  if (_.isString(onlyDelpoyTriggerName)) {
    needDeployTriggers = sourceTriggers.filter(
      ({ name }) => name === onlyDelpoyTriggerName
    );
